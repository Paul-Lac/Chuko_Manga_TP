...
import "./AdminUpdateManga.css";

function AdminUpdateManga({ id }) {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        // console.info("Publishers are", response.data);
  ...
  // Fetching database values for each input
  useEffect(() => {
    axios
      .get(`http://localhost:3310/api/mangas/${id}`)
      .then((res) => {
        console.info("Selected Manga:", res.data);
        setMangaTitle(res.data[0].title);
        setDescription(res.data[0].description);
        setReleaseDate(res.data[0].release_date);
        setAuthor(res.data[0].author);
        setScriptWriter(res.data[0].script_writer);
        setIllustrator(res.data[0].illustrator);
        setPublishingHouse(res.data[0].publishing_house_id);
        setGenre(res.data[0].genre_id);
        setIsFinishedJapan(res.data[0].finish_japan);
        setIsFinishedFrance(res.data[0].finish_france);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(`http://localhost:3310/api/mangas/${id}`, formData)
      .then((res) => {
        console.info("Manga updated successfully", res.data);
        // setTabShow(true);
      })
      .catch((error) => {
        console.error("Error updated manga", error);
      });
  };
    ...
    <div className="manga-form-container">
      ...
      <div className="container-form">
        ...
        <div className="item-to-select">
          ...
          >
            <option value="">Maison d'édition</option>
